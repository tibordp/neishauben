...
rubiks_pos get_position(int X, int Y, int c_size)
{
  ...
  if (((X - 2 - W) % (c_size + 1) == 0) ^ ((X - 2 - W) % (c_size + 1) == c_size - 1) ^
      ((Y - 2 - Q) % (c_size + 1) == 0) ^ ((Y - 2 - Q) % (c_size + 1) == c_size - 1))
  {
    temp.border = 1;
  }
...
int cube_color(cube source, rubiks_pos pos_pp)
{
  ...
  if ((pos_pp.blank == 1) | ((pos_pp.Q_1 < 2) & ((pos_pp.W_1 % 2) == 0)) | ((pos_pp.Q_1 == 3) & ((pos_pp.W_1 % 2) == 0))) {
    return 3;
  } else if (pos_pp.border == 1) {
    return -1;
  } else
  {
    if ((pos_pp.Q_1 == 0) & (pos_pp.W_1 == 1)) {
      return source.D[seq];
}
    if ((pos_pp.Q_1 == 1) & (pos_pp.W_1 == 1)) {
      return source.B[seq];
}
    if ((pos_pp.Q_1 == 2) & (pos_pp.W_1 == 0)) {
      return source.L[seq];
}
    if ((pos_pp.Q_1 == 2) & (pos_pp.W_1 == 1)) {
      return source.U[seq];
}
    if ((pos_pp.Q_1 == 2) & (pos_pp.W_1 == 2)) {
      return source.R[seq];
}
    if ((pos_pp.Q_1 == 3) & (pos_pp.W_1 == 1)) {
      return source.F[seq];
}
  }
}
